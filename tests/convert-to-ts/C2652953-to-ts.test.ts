...
it("should render parent component name for C2652953", async () => {
  const result = `import type { ChipProps } from "@tscircuit/props"
...
<courtyardoutline outline={[{"x":-3.90950500000001,"y":3.143568000000002},{"x":3.8802949999999896,"y":3.143568000000002},{"x":3.8802949999999896,"y":-2.7666320000000013},{"x":-3.90950500000001,"y":-2.7666320000000013},{"x":-3.90950500000001,"y":3.143568000000002}]} />
  ...
  expect(result).toContain("<silkscreentext text={props.name}")
  expect(
    circuitJson.filter(
      (element: any) => element.type === "pcb_silkscreen_text",
    ),
  ).toContainEqual(expect.objectContaining({ text: "U_LC1" }))
